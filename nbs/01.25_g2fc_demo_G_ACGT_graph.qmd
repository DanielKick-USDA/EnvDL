...
## Training a Tiny Arbitrary Graph as a Proof of Principle
    ...
    def __init__(self, 
        ...
        for key in dependancy_order:
            if key in input_tensor_names:
                ...
                layer_list += [Linear_block(in_size=example_dict_input_size[key], 
...
model, loss_df = train_nn_yx(
    cache_path,
    training_dataloader,
    training_dataloader, # Using the training dataloader as the testing dataloader
    model,
    learning_rate = 1e-3,
...
```{python}
#FIXME overwrite with -1, 1 so there is a massive and clear signal. 
for i in range(len(x_list_temp)):
    x_list_temp[i][yield_xbar_summary[0][0], ] = -1
    x_list_temp[i][yield_xbar_summary[1][0], ] =  1
```
...
```{python}
# [e.shape for e in next(iter(training_dataloader))[1] if len(e.shape)]
```
