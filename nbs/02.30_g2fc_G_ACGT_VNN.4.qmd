...
> This is acting as a negative control. I'm randomizing the genetic lookup before training so there should be no good information there. 
...
```{python}
cache_path = '../nbs_artifacts/02.30_g2fc_G_ACGT_VNN.4/'
...
```{python}
obs_geno_lookup_scrambled = obs_geno_lookup.copy()
rng = np.random.default_rng(39485)

obs_geno_lookup_scrambled[:, 1] = rng.choice(
    obs_geno_lookup_scrambled[:, 1], 
    obs_geno_lookup_scrambled.shape[0])

obs_geno_lookup_scrambled
```
...
training_dataloader = DataLoader(ListDataset(
        ...
        obs_idxs = train_idx, 
        obs_geno_lookup = obs_geno_lookup_scrambled
    ),
...
logger = TensorBoardLogger("tb_logs", name="vnn-02.30")                # 3. Update
