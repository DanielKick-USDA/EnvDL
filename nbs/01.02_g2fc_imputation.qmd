...
if 'wthr_powr_imp.csv' in os.listdir(cache_path):
    ...
    for impute_env in impute_envs:
        ...
        wthr_slice = wthr.loc[wthr_mask, ['Env', 'Year', 'Date']]
        # if all null then drop date. This part exists for records with no weather information. 
        if wthr_slice.Date.isna().mean():
            wthr_slice = wthr_slice.loc[:, ['Env', 'Year']]
            wthr_slice = pd.concat([wthr_slice.reset_index(drop = True), 
                                    powr_dl.loc[:, ['Latitude', 'Longitude']].drop_duplicates()], axis = 1)
        wthr_slice = wthr_slice.merge(powr_dl, how = 'outer')        
...
```{python}
            # # [demo.train['x'][e] for e in list(demo.train['x'])]
            # demo.train['x'].loc[demo.train['x']['QV2M'].isna(), ]
...
```{python}
            ...
            # # how is it possible that wthr has no missing values but that train_x does?
            # [(wthr[e].isna().sum(), e) for e in list(wthr)]



            # # how is it possible that wthr has no missing values but that train_x does?
            # [(meta[e].isna().sum(), e) for e in list(meta)]

            # # rows where the problem is
            # set(np.argwhere(np.isnan(train_x))[:, 0])

            # [np.isnan(e).sum() for e in [train_y, train_x, test_y, test_x]]
```
...
# bst.fit(train_x_2d, train_y_1d)
# mean_squared_error(train_y_1d, bst.predict(train_x_2d), squared=False)
```
...
phno.to_csv(cache_path+'phno0.csv')
meta.to_csv(cache_path+'meta0.csv')
soil.to_csv(cache_path+'soil0.csv')
wthr.to_csv(cache_path+'wthr0.csv')
cgmv.to_csv(cache_path+'cgmv0.csv')
```
...
# for i in temp.index:
#    
#     temp_xs = list(temp.loc[(temp.index == i), 'Date_Planted'])+list(temp.loc[(temp.index == i), 'Date_Harvested'])
#     temp_ys = list(temp.loc[(temp.index == i), 'Env'])+list(temp.loc[(temp.index == i), 'Env'])
#    
#     fig.add_trace(go.Scatter(x=temp_xs, y = temp_ys,
#                     mode='lines+markers', 
#                     name= temp_ys[0]
#                             ))
# fig.show()
...
#    
#     temp_xs = list(temp.loc[(temp.index == i), 'Date_Planted'])+list(temp.loc[(temp.index == i), 'Date_Harvested'])
#     temp_ys = list(temp.loc[(temp.index == i), 'Date_Planted'])+list(temp.loc[(temp.index == i), 'Date_Planted'])
#     temp_envs = list(temp.loc[(temp.index == i), 'Env'])+list(temp.loc[(temp.index == i), 'Env'])
#    
#     fig.add_trace(go.Scatter(x=temp_xs, y = temp_ys,
#                     mode='lines+markers', 
#                     name= temp_envs[0]
...
```{python}
# px.histogram(temp, 'Date_Planted')
```

```{python}
# px.scatter(temp, 'Date_Planted', 'DOY')
```
...
```{python}
wthr_temp = wthr.merge(temp, how = 'outer')
wthr_temp
```

```{python}
mask = ((wthr_temp.DOY >= wthr_temp.Start_Date) & (wthr_temp.DOY < wthr_temp.End_Date))
wthr_temp = wthr_temp.loc[mask, ].copy()
```



```{python}
wthr_temp['WindowDate'] = np.nan
wthr_temp['WindowDate'] = 1 + (wthr_temp['DOY'] - wthr_temp['Start_Date'])
```

```{python}
wthr_temp = wthr_temp.drop(columns = [e for e in list(wthr_temp) if e in ['Year', 'Date', 'DOY', 'Start_Date', 'End_Date']])
```
...
```{python}
wthr_temp = wthr_temp.melt(id_vars = ['Env', 'Date_Planted', 'WindowDate'])
```

```{python}
wthr_temp['variable'] = wthr_temp['variable'] +'_Day'+ wthr_temp['WindowDate'].astype(str)
```


```{python}
wthr_temp = wthr_temp.drop(columns = ['WindowDate'])
```

```{python}
wthr_temp = wthr_temp.drop_duplicates()
wthr_temp
```


```{python}
wthr_temp_long = wthr_temp.loc[:, ].pivot(index = ['Env', 'Date_Planted'], columns=['variable']).reset_index()
```
...
wthr_temp = wthr_temp.loc[:, ].pivot(index = ['Env'], columns=['variable']).reset_index()
```

```{python}
test = summarize_col_missing(wthr_temp)
