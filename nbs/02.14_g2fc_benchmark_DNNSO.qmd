...
```{python}
cache_path = '../nbs_artifacts/02.14_g2fc_benchmark_DNNSO/'
...
```{python}
class DNNCO_SO(nn.Module):
    def __init__(self):
        super(DNNCO_SO, self).__init__()  
    ...
    def forward(self, tensor_list):
        ...
        x_pred = self.x_network(torch.concat([g_out, s_out, w_out], axis = 1))
        return x_pred, None
...
model = DNNCO_SO().to(device)
LSUV_(model, data = next(iter(training_dataloader)) )
```

```{python}
DNNCOSO = plDNNCO_subnet(model)                          # 1. Update
optimizer = DNNCOSO.configure_optimizers(lr = lr, betas=(beta1, beta2)) # 2. Update

logger = TensorBoardLogger("tb_logs", name="dnnso")                # 3. Update
trainer = pl.Trainer(max_epochs=max_epoch, logger=logger)

trainer.fit(model=DNNCOSO, train_dataloaders=training_dataloader, val_dataloaders=validation_dataloader)       # 4. Update
```

```{python}
torch.save(DNNCOSO.mod, cache_path+'dnnco-so'+'.pt')
```
