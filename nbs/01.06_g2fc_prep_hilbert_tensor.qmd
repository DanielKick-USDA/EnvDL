...
```{python}
cache_path = '../nbs_artifacts/01.06_g2fc_prep_hilbert_tensor/'
...
YMat = np.load(load_from+'YMat3.npy')
GMat = np.load(load_from+'GMat3.npy')
...
```{python}
# no longer needed. Dropped missing values in 01.03
...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.x_network = nn.Sequential(
            Linear_block(in_size = 2250, out_size = 512, drop_pr = 0.5),
